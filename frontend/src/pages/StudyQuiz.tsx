  ...
  return (
    <div className="study-quiz">
      <header className="sq-topbar">
        <button className="sq-home-button" type="button" onClick={() => navigate('/')} aria-label="홈으로 이동">
          <svg viewBox="0 0 24 24" aria-hidden="true">
            <path d="M4 11.2 12 4l8 7.2v8.6a1 1 0 0 1-1 1h-5.1v-6.2h-3.8v6.2H5a1 1 0 0 1-1-1v-8.6Z" />
          </svg>
        </button>
        <img className="sq-logo" src="/svg/logo.png" alt="on-kid" />
        <div className="sq-profile" aria-label="사용자 정보">
          <span className="sq-profile-name">사민경</span>
          <span className="sq-profile-type">어린이</span>
          <img src="/svg/initmonkey.png" alt="" className="sq-profile-avatar" />
        </div>
      </header>

      <main className="sq-stage" aria-live="polite">
        <section className="sq-question-area">
          <h1>{book}</h1>
          <div className="sq-question-card">
            {visibleQuestion}
          </div>
          <div className="sq-dot-row" aria-label={`진행 ${activeDotCount}/16`}>
            {progressDots.map((active, index) => (
              <span key={index} className={`sq-dot ${active ? 'active' : ''}`} />
            ))}
          </div>
        </section>

        <aside className="sq-command-card" aria-label="퀴즈 명령">
          <button className="sq-menu-button replay" type="button" onMouseEnter={() => speakMenuName('다시')} onClick={() => applyDialog(makeLocalDialog('다시'))}>
            다시
          </button>
          <button className="sq-menu-button previous" type="button" onMouseEnter={() => speakMenuName('이전')} onClick={() => applyDialog(makeLocalDialog('이전'))}>
            이전
          </button>
          <button className="sq-menu-button next" type="button" onMouseEnter={() => speakMenuName('다음')} onClick={() => applyDialog(makeLocalDialog('다음'))}>
            다음
          </button>
          <button className="sq-menu-button hint" type="button" onMouseEnter={() => speakMenuName('힌트')} onClick={() => applyDialog(makeLocalDialog('힌트'))}>
            힌트
          </button>
          <button className="sq-menu-button explain" type="button" onMouseEnter={() => speakMenuName('단어 설명')} onClick={() => applyDialog(makeLocalDialog('단어 설명'))}>
            단어 설명
          </button>
          <button className="sq-menu-button quiz" type="button" onMouseEnter={() => speakMenuName('문제')} onClick={askQuiz}>
            문제
          </button>
        </aside>
      </main>
